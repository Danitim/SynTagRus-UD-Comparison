...
def _eq_forms(a: Optional[str], b: Optional[str]) -> bool:
    ...
    # эквивалентность коротких предлогов
...
def _detach_edge_quotes_in_str(str_sent: Sentence) -> Sentence:
    ...
    qcnt = 0  # счётчик синтетических id для кавычек

    for t in str_sent:
        form = t.form or ''
        if not form:
            # пустая форма — переносим как есть, сохранив исходный id
        ...
        if not leading and not trailing:
            # ничего не прилипло — переносим как есть (сохранив исходный id!)
            new_tokens.append(_mk(form, t.upos,
                                  str(t.head) if t.head not in (None, '_') else '0', t.deprel,
                                  lemma=t.lemma, xpos=t.xpos, feats=t.feats,
                                  id_value=t.id))
        ...
        # 1) ведущие кавычки -> отдельные PUNCT с уникальными id
        for q in leading:
            qcnt += 1
            new_tokens.append(_mk(q, 'PUNCT', '0', 'punct', id_value=f"q{qcnt}"))

        # 2) ядро слова — обязательно с исходным id и исходной головой
        if core:
        ...
        # 3) хвостовые кавычки -> отдельные PUNCT с уникальными id
        for q in trailing:
            qcnt += 1
            new_tokens.append(_mk(q, 'PUNCT', '0', 'punct', id_value=f"q{qcnt}"))
...
def normalize_str_sentence_to_ud(ud_sent: Sentence, str_sent: Sentence) -> Optional[Sentence]:
    ...
    _detach_edge_quotes_in_str(str_sent)
    ...
    while j < len(ud_core):
        ...
        # пропуск лишнего '+' в STR, если UD сейчас ждёт не '+'
        if _is_plus_token(s1) and not _eq_forms(s1.form, u1.form):
            i += 1
            continue

        # --- (A) эллипсис должен выравниваться 1↔1 ---
        if _is_ellipsis(u1):
            if _is_ellipsis(s1):
        ...
        # 2) 2→1 (включая дефисную перестановку)
        ...
        # 3) 3→1 (не трогаем эллипсис внутри окна)
        ...
        # UD-glue (кавычки и т.п.) — как раньше, но без втягивания эллипсиса
        ...
        # локальные перестановки — как раньше (можно оставить твой код), но не перескакивать эллипсис
        moved = False
        WINDOW = 6
        for k in range(1, min(WINDOW, len(str_core) - i)):
            if _is_ellipsis(str_core[i+k]):  # эллипсис не двигаем
                break
    ...
    def _mk_row_word(form: str, upos: str, head_core_idx: Optional[int], deprel: str, ud_id: str) -> dict:
        return {
            ...
            "form": form or "_",
            "upos": upos or "_",
            "head_core_idx": head_core_idx,   # индекс в core_rows или None (root)
            "deprel": (deprel or "_"),
            "_ud_id": ud_id,                  # исходный UD id этого слова
            "_new_id": None,
        }
    ...
    def _mk_row_punct(form: str, anchor_core_idx: Optional[int], ud_head_udid: str) -> dict:
        return {
            ...
            "form": form or "_",
            "upos": "PUNCT",
            "anchor_core_idx": anchor_core_idx,  # к какому слову привязать (индекс скелета), None => потом найдём соседей
            "deprel": "punct",
            "_ud_head_udid": ud_head_udid,       # UD-id головы пункта (на случай фолбэка)
            "_new_id": None,
        }

    # 0) берём UD токены и выделяем ядро UD (слова/эллипсис) + позиционные карты
    ...
    # Карта STR id -> core-индекс UD-токена, к которому выровнялся этот STR-токен.
    # ВАЖНО: сюда кладём ids для ВСЕХ STR-токенов группы (не только «репрезентативного»),
    # чтобы «проброс» головы через склейку работал.
    ...
    # Полная карта голов по STR (включая пунктуацию) для подъёма по цепочке
    ...
    # Быстрые карты по UD ядру
    udid_to_core_idx: Dict[str,int] = {t.id: k for k,t in enumerate(ud_core_tokens)}
    # для фолбэка UD-родителя
    udid_to_ud_headid: Dict[str,str] = {t.id: (str(t.head) if t.head not in (None,'_') else '0') for t in ud_tokens}

    # 1) строим core_rows (только слова/эллипсис) в UD-порядке, головы из STR -> индексы core
    core_rows: List[dict] = []
    ...
    for k, (u_tok, s_tok) in enumerate(zip(ud_core_tokens, aligned_pairs)):
        core_rows.append(_mk_row_word(
            form = u_tok.form,                # форма — как в UD
            upos = s_tok.upos,                # морфология — как в STR
            head_core_idx = None,             # пока заполним ниже
            deprel = s_tok.deprel,
            ud_id = u_tok.id
        ))
        strid_to_core_idx[str(s_tok.id)] = k

    # проставляем head_core_idx для слов, опираясь ТОЛЬКО на структуру STR
    # (ходим по цепочке head в STR, пропуская пунктуацию и внутренние узлы своей «группы»,
    # пока не найдём внешний узел, который тоже выровнен в ядро)
    for k, (u_tok, s_tok) in enumerate(zip(ud_core_tokens, aligned_pairs)):
        # Все STR-участники этой UD-позиции (2→1, 3→1 и т.п.) — по link_map
        ud_here = u_tok.id
        group_str_ids = [sid for sid, uid in link_map.items() if uid == ud_here]

        # стартовая STR-голова — у «репрезентативного» s_tok
        cur = str(s_tok.head) if s_tok.head not in (None, "", "_") else "0"

        # Поднимемся по STR-цепочке головы, пока:
        # - не выйдем к '0', или
        # - не найдём STR-узел, сопоставленный любому ДРУГОМУ core-индексу (внешний по отношению к нашей группе).
        visited: set[str] = set()
        ...
        while cur != "0" and cur not in visited:
            ...
            # если «голова» — кто-то из нашей же группы (внутренний), поднимаемся выше
            if cur in group_str_ids:
                cur = strid_to_headid.get(cur, "0")
                continue

            # если «голова» выровнена в ядро — проверим, что это НЕ мы сами (не свой core-индекс)
            if cur in strid_to_core_idx_all:
                cand_core = strid_to_core_idx_all[cur]
                if cand_core != k:
                    found_core = cand_core
                    break
                # иначе это та же UD-позиция (самоссылка) — поднимемся выше
                cur = strid_to_headid.get(cur, "0")
                continue

            # иначе: узел не выровнен (например, пунктуация) — поднимаемся выше по STR
            cur = strid_to_headid.get(cur, "0")

        # Назначаем head_core_idx исходя из найденного внешнего ядра
        if found_core is None:
            # Спец-фолбэк ТОЛЬКО для ASCII 'x': берём голову из UD, чтобы не делать её root
    ...
    # 2) вставляем пунктуацию из UD между узлами скелета
    #    пройдём по полному UD-порядку и «эмитим» либо слово (просто брали из core_rows), либо пунктуацию
    out_rows: List[dict] = []
    core_cursor = 0
    # карта: ud_id слова -> index в out_rows (заполним по мере эмиссии)
    udid_to_out_index: Dict[str,int] = {}

    # helper: вставить пунктуацию, якорь — по UD-голове, если это слово; иначе ближайшее слово слева/справа
    def _emit_punct_from_ud(u_punct: Token):
        ...
        if ud_head != '0' and ud_head in udid_to_core_idx:
            # знаем слово-голову по UD -> его core-индекс
            anchor_core = udid_to_core_idx[ud_head]
        else:
            # якорь неизвестен в ядре: положим None, привяжем позже по соседям
            anchor_core = None
    ...
    for i, u in enumerate(ud_tokens):
        if _is_punct(u):
            ...
            # это слово из ядра; «эмитим» соответствующую core_row
            row = core_rows[core_cursor]
            out_rows.append(dict(row))  # копию
            udid_to_out_index[u.id] = len(out_rows)-1
            core_cursor += 1

    # 3) теперь у нас смешанный список out_rows; присвоим окончательные ID
    for new_id, r in enumerate(out_rows, start=1):
        r["_new_id"] = str(new_id)

    # 4) финально переводим head_core_idx/anchor_core_idx в новые id и чиним сирот
    def _coreidx_to_newid(core_idx: Optional[int]) -> Optional[str]:
        if core_idx is None:
            return None
        # найдём в out_rows позицию соответствующего core-слова:
        # пройдём и сравним _ud_id (у слов он есть)
    ...
    # сначала проставим головы у слов
    for r in out_rows:
        if r["kind"] != "word":
            continue
        if r["head_core_idx"] is None:
            ...
            # если вдруг не нашли новый id или вышла самоссылка — делаем корнем
    ...
    # затем пунктуацию: якорь по anchor_core_idx, иначе ближайший словесный сосед
    for idx, r in enumerate(out_rows):
        if r["kind"] != "punct":
            continue
        nid = _coreidx_to_newid(r["anchor_core_idx"])
        if nid is None:
            # найдём ближайшее слово справа, затем слева
            anchor = None
            ...
            r["head"] = nid
        # deprel для пунктуации
        if not r["deprel"] or r["deprel"].lower() == "root":
            r["deprel"] = "punct"

    # 5) собираем итоговый Sentence
