...
# 4. Train UDPipe 2 models for all corpora (morphology + syntax in one model)
echo "=== [STEP 4] Training UDPipe 2 models (morph + syntax) ==="
mkdir -p models

train_tag_parse () {
  local corpus="$1"
  local model_dir="models/${corpus}-morphsyntax"
  ...
  echo "[train] tag+parse corpus=${corpus}"
...
for corpus in "${CORPORA[@]}"; do
  train_tag_parse "$corpus"
done

echo "=== [ALL DONE] All tag+parse models trained successfully ==="
exec bash
