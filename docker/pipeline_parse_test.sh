...
# 2. Compute wembeddings for test dataset
echo "=== [STEP 2] Computing wembeddings for test corpus ==="
...
for split in train dev test; do
  compute_wemb "datasets/test/${split}.conllu"
done
...
# 4. Train UDPipe 2 parsing-only model for test corpus (minimal epochs)
echo "=== [STEP 4] Training UDPipe 2 parsing-only model (test) ==="
mkdir -p models

train_file="datasets/test/train.conllu"
dev_file="datasets/test/dev.conllu"
model_dir="models/test-parse"

if [[ ! -f "$train_file" ]]; then
  echo "[skip] $train_file not found, skipping test training"
  exit 0
fi

echo "[train] corpus=test"
...
echo "=== [ALL DONE] Test parsing-only model trained successfully ==="
exec bash
